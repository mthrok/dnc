...
from torch_dnc.addressing import (
    weighted_softmax,
    CosineWeights,
    TemporalLinkage,
    Freeness
...
@pytest.mark.parametrize("strength_op", [
    lambda x: x,
    F.softplus,
])
def test_weighted_softmax(strength_op):
...
def test_temporal_linkage():
    ...
    torch.testing.assert_close(
        state.link[0, 1, :, 3],
        one_hot(memory_size, 2, dtype=torch.float32))
...
def test_freeness_write_allocation_weights():
    ...
    torch.testing.assert_close(
        weights.sum(dim=2),
        torch.ones((batch_size, num_writes)), atol=1e-3, rtol=0)
    ...
    torch.testing.assert_close(
        weights[1][0],
        one_hot(memory_size, 4, dtype=torch.float32),
        atol=1e-3, rtol=0)
    torch.testing.assert_close(
        weights[1][1],
        one_hot(memory_size, 3, dtype=torch.float32),
...
def test_freeness_write_allocation_weights_gradient():
    ...
    usage = torch.rand((batch_size, memory_size), requires_grad=True)
    write_gates = torch.rand((batch_size, num_writes), requires_grad=True)
...
def test_freeness_allocation():
    ...
    torch.testing.assert_close(allocation.sum(dim=1), torch.ones(batch_size), atol=0.01, rtol=0)
