...
from torch_dnc.access import MemoryAccess, AccessState
from torch_dnc.addressing import TemporalLinkageState
...
def test_memory_access_valid_read_mode():
    ...
    # distribution.
    torch.testing.assert_close(inputs['read_mode'].sum(2), torch.ones([BATCH_SIZE, NUM_READS]))
...
def test_memory_access_read_weights():
    ...
    read_content_strengths = torch.full((BATCH_SIZE, NUM_READS), 100., dtype=torch.float64)
...
def test_memory_access_gradients():
    inputs = torch.randn((BATCH_SIZE, INPUT_SIZE), requires_grad=True)
    memory = torch.zeros((BATCH_SIZE, MEMORY_SIZE, WORD_SIZE))
    read_weights = torch.zeros((BATCH_SIZE, NUM_READS, MEMORY_SIZE))
    write_weights = torch.zeros((BATCH_SIZE, NUM_WRITES, MEMORY_SIZE))
    link = torch.zeros((BATCH_SIZE, NUM_WRITES, MEMORY_SIZE, MEMORY_SIZE))
    precedence_weights = torch.zeros((BATCH_SIZE, NUM_WRITES, MEMORY_SIZE))
    usage = torch.zeros((BATCH_SIZE, MEMORY_SIZE))

    module = MemoryAccess(MEMORY_SIZE, WORD_SIZE, NUM_READS, NUM_WRITES)

    
    def func(inputs, memory, read_weights, write_weights, link, precedence_weights, usage):
        state = AccessState(
            memory=memory,
        ...
        output, _ = module(inputs, state)
        return output

    # NOTE: atol is tighter than the original implementation
    torch.autograd.gradcheck(
        func,
        (inputs, memory, read_weights, write_weights, link, precedence_weights, usage),
        eps=0.001,
        atol=1e-05,
        rtol=0,
    )
