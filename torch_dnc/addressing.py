...
class TemporalLinkage(torch.nn.Module):
    ...
    def _link(
        ...
        # edges.
        mask = torch.eye(self._memory_size).repeat(batch_size, self._num_writes, 1, 1).bool()
        link[mask] = 0
        return link
    ...
    def forward(
        ...
        if write_weights.size(1) != self._num_writes or write_weights.size(2) != self._memory_size:
            raise ValueError(
