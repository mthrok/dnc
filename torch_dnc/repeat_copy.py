...
class RepeatCopy:
    ...
    def __call__(self):
        ...
        for batch_index in range(batch_size):
            ...
            obs_pattern = torch.randint(low=0, high=2, size=obs_pattern_shape).to(torch.float32)
            ...
            obs_start_flag = F.one_hot(
                torch.tensor([start_end_flag_idx]), num_classes=full_obs_size).to(torch.float32)
            num_reps_flag = F.one_hot(
                torch.tensor([num_repeats_channel_idx]), num_classes=full_obs_size).to(torch.float32)
            ...
            targ_end_flag = F.one_hot(
                torch.tensor([start_end_flag_idx]), num_classes=full_targ_size).to(torch.float32)
