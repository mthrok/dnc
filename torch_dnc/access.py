...
class MemoryAccess(torch.nn.Module):
    ...
    *   `addressing.FreenessAllocator` for keeping track of memory usage, where
    ...
    def __init__(
        ...
        self._read_mode = _linear(num_reads, num_read_modes, post_act=torch.nn.Softmax(dim=-1))
    ...
    def forward(
        ...
        if prev_state is None:
            ...
            prev_state = AccessState(
                memory=torch.zeros((B, self._memory_size, self._word_size), device=device),
                read_weights=torch.zeros((B, self._num_reads, self._memory_size), device=device),
                write_weights=torch.zeros((B, self._num_writes, self._memory_size), device=device),
                linkage=None,
